...
test("shared shell assets are public", async () => {
  ...
  assert.match(cssText, /xm-phone-panel 0\.1\.139/);
  ...
  assert.match(jsText, /xm-fast-shell 0\.1\.139/);
...
test("page renderer injects shared shell onto module html", async () => {
  ...
  assert.match(injected, /\/shared\/nav\.js\?v=0\.1\.139/);
  assert.match(injected, /rel="preload" href="\/shared\/nav\.js\?v=0\.1\.139"/);
  ...
  assert.match(mid, /SHELL_ASSET_VER = "0\.1\.139"/);
  ...
  assert.match(shell, /rel="icon" type="image\/png" sizes="32x32" href="\/shared\/tab-icon\.png\?v=0\.1\.139"/);
  assert.match(shell, /rel="shortcut icon" href="\/favicon\.ico\?v=0\.1\.139"/);
