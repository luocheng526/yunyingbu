...
test("shared shell assets are public", async () => {
  ...
  const versionedJs = await fetch(`${base}/shared/nav.js?v=0.1.64`);
  ...
  assert.match(jsText, /xm-shell-pages 0\.1\.64/);
  ...
  assert.match(jsText, /function pinUpgradeMask\(/);
  assert.match(jsText, /document\.documentElement\.appendChild/);
  assert.match(jsText, /function wrapHanFetch\(/);
  assert.match(jsText, /getAttribute\("data-self"\) === "1"/);
  assert.match(jsText, /keepalive: true/);
  assert.match(jsText, /sessionStorage\.removeItem\("xm-me"\)/);
  assert.doesNotMatch(jsText, /warmBusy/);
  assert.doesNotMatch(jsText, /function navigate\(/);
  assert.doesNotMatch(jsText, /function warmAppPages\(/);
  assert.doesNotMatch(jsText, /function cachedHtml\(/);
  assert.doesNotMatch(jsText, /function bindSpa\(/);
  assert.doesNotMatch(jsText, /history\.pushState/);
  assert.doesNotMatch(jsText, /AbortController/);
...
test("page renderer injects shared shell onto module html", async () => {
  ...
  assert.match(injected, /\/shared\/nav\.js\?v=0\.1\.64/);
  assert.match(injected, /rel="preload" href="\/shared\/nav\.js\?v=0\.1\.64"/);
