...
test("shared shell assets are public", async () => {
  ...
  assert.match(jsText, /function paneHasModule/);
  assert.match(jsText, /function homePaneEmpty/);
  assert.match(jsText, /function bootHomePane/);
  assert.match(jsText, /function injectModuleScript/);
  const manifest = await fetch(`${base}/manifest.webmanifest`);
  assert.equal(manifest.status, 200);
  const manifestJson = await manifest.json();
  assert.equal(manifestJson.start_url, "/home");
  assert.equal(manifestJson.display, "standalone");
  assert.equal(manifestJson.icons[0].src, "/apple-touch-icon.png?v=0.1.146");
  ...
  assert.match(cssText, /xm-phone-panel 0\.1\.145/);
  assert.match(cssText, /#xm-hm-ai/);
  assert.match(cssText, /#xm-hm-ai-refresh/);
  assert.match(cssText, /@media \(max-width: 880px\)/);
  assert.match(cssText, /html\.xm-phone-nav \.xm-sider/);
  assert.match(cssText, /#xm-hm \.xm-hm-views/);
  assert.match(cssText, /#xm-hm \.xm-hm-ranges/);
  assert.match(cssText, /data-range="yesterday"/);
  assert.match(cssText, /data-range="lastMonth"/);
  assert.match(cssText, /content: "自定义"/);
  assert.match(cssText, /repeat\(3, minmax\(0, 1fr\)\)/);
  assert.match(cssText, /html\.xm-phone-cal/);
  assert.match(cssText, /position: fixed !important/);
  assert.match(cssText, /\.xm-phone-cal-bar/);
  assert.match(jsText, /function phonePanelCssText/);
  assert.match(jsText, /function compactPhoneHomeDates/);
  assert.match(jsText, /function watchPhoneHomeDates/);
  assert.match(jsText, /function decoratePhoneCal/);
  assert.match(jsText, /function watchPhoneHomeCal/);
  assert.match(jsText, /function watchHomeAiDiag/);
  assert.match(jsText, /function renderHomeAiDiag/);
  assert.match(jsText, /AI 诊断/);
  assert.match(jsText, /#xm-hm-ai-refresh/);
  assert.match(jsText, /function armPhoneCalDouble/);
  assert.match(jsText, /连点同一天只看当天/);
  assert.match(jsText, /#xm-hm \.xm-hm-bar/);
  assert.match(jsText, /function scriptHasFinished/);
  assert.match(jsText, /#xm-hm, \.xm-hm/);
  assert.match(jsText, /href === "\/home" && homePaneEmpty/);
  ...
  assert.match(jsText, /xm-fast-shell 0\.1\.146/);
  ...
  assert.match(jsText, /items\.slice\(0, 7\)/);
  ...
  const storesAt = itemsBlock.indexOf('href: "/stores"');
  ...
  assert.ok(storesAt > -1 && academyAt > storesAt && agentsAt > academyAt && releasesAt > agentsAt);
...
test("home page html is the xingmai sider template", async () => {
  ...
  assert.match(html, /v0\.4\.24/);
  assert.match(html, /店铺维护中心/);
...
test("甄选商学院 expands three children and 甄选智能体 stays a leaf", async () => {
  ...
  assert.match(academyHtml, /店铺维护中心/);
  ...
  assert.match(mainNav, /href="\/stores"/);
  assert.match(mainNav, /店铺维护中心/);
  assert.match(mainNav, /data-xm-group="\/academy"/);
  assert.ok(mainNav.indexOf('href="/stores"') < mainNav.indexOf('data-xm-group="/academy"'));
  ...
  const stores = await fetch(`${base}/stores`, { headers: { cookie } });
  assert.equal(stores.status, 200);
  const storesHtml = await stores.text();
  assert.match(storesHtml, /\/shared\/modules\/stores\.js/);
  assert.match(storesHtml, /店铺维护中心/);
  ...
  const storesMod = readFileSync(join(root, "public/shared/modules/stores.js"), "utf8");
  ...
  assert.match(storesMod, /XmModules\["\/stores"\]/);
  ...
  const storesApi = await fetch(`${base}/api/stores`, { headers: { cookie } });
  assert.equal(storesApi.status, 200);
  assert.equal((await storesApi.json()).module, "店铺维护中心");
...
test("page renderer injects shared shell onto module html", async () => {
  ...
  assert.match(injected, /\/shared\/nav\.js\?v=0\.1\.146/);
  assert.match(injected, /rel="preload" href="\/shared\/nav\.js\?v=0\.1\.146"/);
  assert.match(injected, /\/apple-touch-icon\.png\?v=0\.1\.146/);
  ...
  assert.match(shell, /rel="icon" type="image\/png" sizes="32x32" href="\/shared\/tab-icon\.png\?v=0\.1\.146"/);
  assert.match(shell, /rel="shortcut icon" href="\/favicon\.ico\?v=0\.1\.146"/);
  assert.match(shell, /rel="apple-touch-icon" sizes="180x180" href="\/apple-touch-icon\.png\?v=0\.1\.146"/);
  ...
  assert.match(shell, /\/shared\/modules\/data\.js/);
  assert.doesNotMatch(shell, /\/shared\/modules\/data\.js\?v=/);
  ...
  assert.match(shell, /getAttribute\("data-xm-href"\)/);
  assert.match(shell, /href !== key/);
