...
test("shell HTML is versioned so browsers drop the old full-reload nav.js", async () => {
  ...
  assert.match(html, /\/shared\/layout\.css\?v=0\.1\.104/);
  assert.match(html, /\/shared\/modules\/data\.js\?v=0\.1\.104/);
...
test("sidebar HTML stays fast when the same process is reused", async () => {
  ...
  const samples = [];
  for (const path of ["/data/overview", "/shen/selection", "/han/selection", "/me", "/han/goods", "/releases"]) {
