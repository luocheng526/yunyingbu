...
test("shell HTML is versioned so browsers drop the old full-reload nav.js", async () => {
  ...
  assert.match(html, /\/shared\/layout\.css\?v=0\.1\.102/);
  assert.match(html, /\/shared\/modules\/data\.js\?v=0\.1\.102/);
...
test("sidebar HTML stays fast when the same process is reused", async () => {
  ...
  const samples = [];
  for (const path of ["/han/selection", "/data", "/me", "/han/goods", "/releases"]) {
