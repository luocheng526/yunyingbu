...
test("ERP shop pages keep fetching until the last short page", async () => {
  ...
  const erp = {
    async erpPost(path, body) {
      calls.push({ path, pageNum: body.pageNum, pageSize: body.pageSize, shopIds: body.shopIds });
      if (body.pageNum === 1) {
  ...
  assert.deepEqual(calls[0].shopIds, []);
});

test("ERP shop pages pass only requested shop ids", async () => {
  const calls = [];
  const erp = {
    async erpPost(_path, body) {
      calls.push(body.shopIds);
      return { total: 1, records: [{ shopId: "111", payAmount: 10 }] };
    }
  };
  const page = await fetchAllShopPages(
    erp,
    { payTimeStart: "2026-09-16 00:00:00", payTimeEnd: "2026-09-16 23:59:59" },
    200,
    ["111"]
  );
  assert.deepEqual(calls[0], [111]);
  assert.equal(page.records[0].shopId, "111");
});
