...
(function () {
  ...
  function ensureShopBoard(root) {
    let board = pageBoard(root, "shops");
    if (board) {
      return board;
    }
    root.innerHTML =
  ...
  function createShopDashboard(root) {
    ...
    const board = ensureShopBoard(root);
    if (!board) {
  ...
  const shopPane = document.querySelector('.xm-pane[data-xm-href="/data/shops"]');
  if (!shopPane && /\/data\/shops\/?$/.test(String(location.pathname || "").replace(/\/index\.html$/, ""))) {
    createShopDashboard(document.querySelector("main.xm-page") || document.body);
  }
})();
