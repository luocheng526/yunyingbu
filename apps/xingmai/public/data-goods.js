/* xm-data-goods 0.1.612-data-range-blank — range change blanks numbers before new data */
(function () {
  ...
  function ensureGoodsBoard(root) {
    let board = pageBoard(root, "goods");
    if (board) {
      return board;
    }
    root.innerHTML =
      '<main class="xm-page data-overview-root ch-root" data-xm-data-page="goods"><div data-board="goods" id="board-goods"></div></main>';
    return root.querySelector('[data-board="goods"]');
  }

  function createGoodsDashboard(root) {
    ensureCss();
    stripPageChrome(root);
    const board = ensureGoodsBoard(root);
    if (!board) {
  ...
  const goodsPane = document.querySelector('.xm-pane[data-xm-href="/data/goods"]');
  if (!goodsPane && /\/data\/goods\/?$/.test(String(location.pathname || "").replace(/\/index\.html$/, ""))) {
    createGoodsDashboard(document.querySelector("main.xm-page") || document.body);
  }
})();
