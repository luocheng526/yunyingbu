  ...
  window.XmModules["/notices"] = {
    mount: function (root) {
      ...
      function paintPanel(data) {
        ...
        panel.innerHTML =
          '<div class="notice-toolbar"><div><h2>进行中的公告</h2><p class="notice-lead">按重要程度和类型筛选。后续可在这里增改。</p></div>' +
      ...
      function load() {
        ...
        const cat = tab === "all" ? "" : (query ? "&" : "") + "category=" + encodeURIComponent(tab);
      ...
      root.addEventListener("click", function (event) {
        ...
        if (event.target.closest("#notice-create")) {
          mode = "form";
          current = { popup: false, banner: true, category: tab === "all" ? "general" : tab };
          paintPanel();
          return;
        }
