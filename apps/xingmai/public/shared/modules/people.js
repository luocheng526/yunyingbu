...
(function () {
  ...
  window.XmModules["/people"] = {
    mount: function (root) {
      ...
      function summaryFromActiveStores(stores) {
        const rows = stores || [];
        const active = rows.filter(countsInStoreStats);
        ...
        return {
          ...
          idle: rows.filter(function (row) {
            return String(row.statusKey || "") === "idle" || String(row.remark || "") === "闲置中";
          }).length,
          closing: rows.filter(function (row) {
            return String(row.statusKey || "") === "closing" || String(row.remark || "") === "退店中";
          }).length,
          closed: rows.filter(function (row) {
            return String(row.statusKey || "") === "closed" || String(row.remark || "") === "已退店";
          }).length,
          missingStoreId: missing("storeId"),
          missingMerchant: missing("merchantId"),
          missingLogin: missing("login"),
          missingPassword: missing("password"),
          missingOwner: active.filter(function (row) {
            return !String(row.operator || row.owner || "").trim();
          }).length
        };
      }
      ...
      function loadBoard() {
        ...
        return Promise.all([
          ...
          renderKpis(summaryFromActiveStores(rawStores));
          paintStores();
          paintFilterCarets();
        });
      }
