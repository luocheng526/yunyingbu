/* xm-data-overview 0.1.607-data-range-blank — range change blanks numbers before new data */
(function () {
  ...
  function ensureOverviewBoard(root) {
    let board = pageBoard(root, "overview");
    if (board) {
    ...
    root.innerHTML =
      '<main class="xm-page data-overview-root ch-root" data-xm-data-page="overview"><div data-board="overview" id="board-overview"><p class="ch-empty">正在加载数据总览…</p></div></main>';
  ...
  function createDashboard(root) {
    ...
    const board = ensureOverviewBoard(root);
    if (!board) {
  ...
  if (!overviewPane && /\/data\/overview\/?$/.test(String(location.pathname || "").replace(/\/index\.html$/, ""))) {
    createDashboard(document.querySelector("main.xm-page") || document.body);
  }
})();
