/* xm-data-overview 0.1.630-data-pane-isolate — keep year-pick; paint only board-overview */
(function () {
  ...
  function ownerHref(root) {
    const pane = root && root.closest && root.closest("[data-xm-href], .xm-pane");
    return pane && pane.getAttribute ? String(pane.getAttribute("data-xm-href") || "") : "";
  }
  ...
  function pageBoard(root) {
    ...
    const owned =
      root.querySelector('[data-board="overview"]') ||
      root.querySelector("#board-overview");
    if (owned) {
      return owned;
    }
    ...
    const page = board.closest("[data-xm-data-page]");
    if (page && page.getAttribute("data-xm-data-page") !== "overview") {
      return null;
    }
    const pane = board.closest("[data-xm-href]");
    if (pane && pane.getAttribute("data-xm-href") !== "/data/overview") {
      return null;
    }
    return board;
  }

  function resolveOverviewRoot(root) {
    if (!root) {
      return null;
    }
    const href = ownerHref(root);
    if (href === "/data/shops" || href === "/data/goods" || href === "/data/paid") {
      return null;
    }
    if (root === document.body || (root.classList && root.classList.contains("xm-workspace"))) {
      return document.querySelector('.xm-pane[data-xm-href="/data/overview"]') || null;
    }
    return root;
  }

  function createDashboard(root) {
    root = resolveOverviewRoot(root);
    if (!root) {
      return function unmount() {};
    }
    ensureCss();
    stripPageChrome(root);
    let board = pageBoard(root);
    if (!board) {
      root.innerHTML = '<main class="xm-page data-overview-root ch-root" data-xm-data-page="overview"><div data-board="overview" id="board-overview"><p class="ch-empty">正在加载数据总览…</p></div></main>';
      board = pageBoard(root);
  ...
  const overviewPane = document.querySelector('.xm-pane[data-xm-href="/data/overview"]');
  const existingBoard =
    (overviewPane &&
      (overviewPane.querySelector('[data-board="overview"], #board-overview, #board'))) ||
    document.querySelector('[data-board="overview"], #board-overview');
  const pane = existingBoard && existingBoard.closest("[data-xm-href], .xm-pane, .xm-page");
  const paneHref = pane && pane.getAttribute ? String(pane.getAttribute("data-xm-href") || "") : "";
  const pageTag = existingBoard && existingBoard.closest("[data-xm-data-page]");
  const pageKind = pageTag ? String(pageTag.getAttribute("data-xm-data-page") || "") : "";
  if (
    existingBoard &&
    /\/data\/overview\/?$/.test(location.pathname) &&
    paneHref !== "/data/shops" &&
    paneHref !== "/data/goods" &&
    pageKind !== "shops" &&
    pageKind !== "goods"
  ) {
    createDashboard(overviewPane || existingBoard.closest(".xm-page") || existingBoard);
  }
})();
