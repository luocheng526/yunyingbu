/* xm-data-overview 0.1.626-data-range-blank — range change blanks numbers before new data */
(function () {
  ...
  function ensureCss() {
    if (!document.querySelector('link[href^="/data-pages.css"]')) {
      ...
      link.rel = "stylesheet";
      link.href = "/data-pages.css?v=0.1.626-data-range-blank";
  ...
  function ensureCardTypeStyle() {
    ...
    style.textContent =
      ...
      ".is-wait .ch-metrics .value,.is-wait .ch-metrics .extra,.is-wait .ch-hero .delta,.is-wait .ch-hero .value,.is-wait .ch-table td,.is-wait .ch-table td *,.is-wait .ch-pill,.is-wait .op-kpi .val{color:transparent!important}" +
      ".is-wait .ch-hero .ch-spark,.is-wait .op-bars,.is-wait .ch-table .ch-bar,.is-wait .ch-table .ch-logo{opacity:0}";
    document.head.appendChild(style);
  }
  ...
  function createDashboard(root) {
    if (root && root.getAttribute && root.getAttribute("data-xm-data-page") && root.getAttribute("data-xm-data-page") !== "overview") {
    ...
    function setWait(on) {
      ...
      if (rootEl) {
        rootEl.classList.toggle("is-wait", state.wait);
      }
      if (board) {
  ...
  const existingBoard = document.querySelector('[data-board="overview"], #board-overview');
  const pane = existingBoard && existingBoard.closest("[data-xm-href], .xm-pane, .xm-page");
  if (existingBoard && /\/data\/overview\/?$/.test(location.pathname) && pane && pane.getAttribute("data-xm-mounted") !== "/data/shops" && pane.getAttribute("data-xm-mounted") !== "/data/goods") {
