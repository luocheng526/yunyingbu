...
test("shen module mounts product and paid content only", async () => {
  await withServer(async (base) => {
    ...
    assert.match(embed.text, /productCenterPage/);
    assert.match(embed.text, /waitPage\("付费中心"\)/);
    assert.match(embed.text, /选品/);
    assert.match(embed.text, /优化/);
    assert.match(embed.text, /产品成长/);
    ...
    assert.match(product.text, /选品/);
    assert.match(product.text, /优化/);
    assert.match(product.text, /产品成长/);
