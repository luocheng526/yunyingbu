...
test("GET /people lists title, table headers and fallback nav", async () => {
  await withServer(async (base) => {
    ...
    const res = await fetch(`${base}/people`, { headers: { Cookie: cookie } });
...
test("GET /api/people returns three demo staff", async () => {
  await withServer(async (base) => {
    const cookie = await loginCookie(base);
    const res = await fetch(`${base}/api/people`, { headers: { Cookie: cookie } });
...
test("POST /api/people appends a staff row", async () => {
  await withServer(async (base) => {
    ...
    const listed = await fetch(`${base}/api/people`, { headers: { Cookie: cookie } });
