...
test("GET /han is 韩梦凯运营中心 with left sidebar shell", async () => {
  await withServer(async (base) => {
    ...
    const res = await fetch(`${base}/han`, { headers: { Cookie: cookie } });
...
test("han tasks default owner is 韩梦凯", async () => {
  await withServer(async (base) => {
    ...
    const listed = await json(base, "/api/han/tasks", { cookie });
...
test("han brief round-trip", async () => {
  await withServer(async (base) => {
    const cookie = await loginCookie(base);
    const empty = await json(base, "/api/han/brief", { cookie });
    ...
    const again = await json(base, "/api/han/brief", { cookie });
...
test("han tasks do not appear on /api/shen/tasks", async () => {
  await withServer(async (base) => {
    ...
    const shen = await json(base, "/api/shen/tasks", { cookie });
...
test("notes demo API still works", async () => {
  await withServer(async (base) => {
    const cookie = await loginCookie(base);
    const empty = await json(base, "/api/notes", { cookie });
