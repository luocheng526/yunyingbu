...
先读 [docs/agents/00-prod-admin.md](docs/agents/00-prod-admin.md)。生产站 `https://zx.xingmaierp.cc` 不是演示环境。交单只用管理员号：用户名 `罗成` 或 `luocheng`。申请人写 `罗成运营部主脑`。不要用演示账号。不要把密码写进单据标题或登录页。本地测试仍用仓库种子密码，不要改 `auth.js` 种子去迁就线上。
