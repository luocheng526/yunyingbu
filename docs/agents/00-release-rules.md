...
- **主框架**：只交壳四件套、内核三件、闸补丁。侧栏由主框架交付，不是首页。升壳见 [00-shell-bump.md](00-shell-bump.md)。
- 各板块：只改自己的目录和 `public/shared/modules/<自己>.js`，做完后 `POST /api/releases` 交单，然后等网页「通过」。要改子菜单时摘要写「请主框架：……」；只改自己的模块脚本不要叫升壳。
...
- 测试、curl、打开页面即可。**不要做视频预览，也不要做图片预览。**
- 只有用户亲口说「要预览图 / 要截图 / 要录像 / 要录屏」时才拍。
- 没说就不要拍：不要为了交单、验收、走查、证明改动、walkthrough 自动截图或录屏。大改动也一样。
...
Cookie: 管理员号登录会话
{
  ...
  "applicant": "罗成运营部主脑",
  "source": "罗成运营部主脑",
  "module": "主框架|首页|数据中心|沈子晗|韩梦凯|组织中心|人员管理|店铺维护中心|甄选商学院|甄选智能体|版本发布中心|个人中心",
  "summary": "更新了什么",
  "files": ["public/...", "src/..."],
