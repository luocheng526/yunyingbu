...
主脑说「网页打不开」时：先说明原因、要改哪些内容、建议修不修，**不要先交单**。主脑回「修复」后，主框架改完立刻交单，并 `POST /api/releases/:id/confirm` 强制通过，不要等主脑去网站点「通过」。若闸门要求先处理队首，驳回挡路的队首（写明是为打开站点），再强制通过打开站点这一单。全文见仓库根目录 [AGENTS.md](../../AGENTS.md)。
