...
【你不能改】壳四件套、`nav-items.js`、`src/app.js`、别人的模块。要加/删/改子菜单：摘要写「请主框架：……」，让用户把这句话丢给主框架对话框。只改 `data.js` 不要叫升壳。见 [00-shell-bump.md](00-shell-bump.md)。
...
【不要做】不要把 ERP token 写进仓库或页面；不要改人员/发布/两个运营中心/个人中心；不要接实时付费看板接口。不要做视频预览或图片预览，除非用户亲口说需要。
