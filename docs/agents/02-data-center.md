...
【要做】
1. 数据总揽：沿用原看板骨架（四张 KPI + 趋势看板 + 实时销售指数 + 龙虎榜）。KPI 填应收/订单/利润/退款。趋势看板填销售趋势。实时销售指数先空着。龙虎榜填店铺排行。不要改标题，不要再加店铺排行表或热销商品块。不要演示「今日订单 / 待处理 / 在职人数 / 本周发布」。
2. 店铺数据：沿用原列表页（店名筛选 + 表 + 翻页）。表头仍是店铺/类型/状态/类目/简介/开店时间，用店铺管理填。不要改成销售列，不要今日/昨日，不要加图。
3. 商品数据：沿用原表头 商品/店铺/订单/应收/净销售/利润/推广。店名用店铺管理补。不要加退款率列。
4. 渠道分组 / 渠道品类 / 渠道对比：同一套列表页，只填表。品类不要另画走势图。
5. 实时付费：后期另接，先保持「内容待开发」。
6. 服务端自己登录 ERP，过期或 401 再登。可用 `XM_ERP_USERNAME` / `XM_ERP_PASSWORD`。不要把 JWT、密码或京东 `authInfo` 交给浏览器。
7. **只填数，不改版式。** 颜色、卡片、折线、列表页骨架都用现成 class，不要另起一套 UI。
