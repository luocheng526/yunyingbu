...
6. 侧栏：窄 `200px`、无「项目」分组、无顶栏退出/暗色按钮。主区是首页 / 数据中心 / 沈 / 韩 / 店铺维护中心 / 甄选商学院 / 甄选智能体。底部是版本发布中心 / 组织中心 / 个人中心 / 退出登录 / 版本号。首页路径 `/home`，`/` 跳 `/home`。
7. 主框架必须认识「店铺维护中心」`/stores`（在甄选商学院上面）、「甄选商学院」（子菜单：培训课程 / 培训考试 / 运营手册）和「甄选智能体」：`NAV_MAIN`、`APP_MODULES`、`MODULES`、章程。新对话框只填内容，不要再铺菜单。
