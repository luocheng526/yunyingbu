...
【纪律】
- 站点：`https://zx.xingmaierp.cc/academy`（会转到 `/academy/courses`）
...
- 嵌入式：`public/shared/modules/academy.js` 挂 `XmModules["/academy/courses"]`、`/academy/exams`、`/academy/handbook`。不要自画侧栏，不要交壳，不要交 `src/app.js`。
...
- 主框架加载 `/shared/modules/academy.js`，你只往 `#xm-content` 画对应子页。
...
【验收】登录后点侧栏「甄选商学院」展开，再点培训课程 / 培训考试 / 运营手册，内容出现在壳的 `#xm-content`，刷新不丢侧栏。
【不要做】不要改侧栏品牌条和 logo；不要把培训塞进沈/韩的「培训系统」子菜单。不要做视频预览或图片预览，除非用户亲口说需要。
