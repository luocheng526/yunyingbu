/** Demo overview. Keeps hydrateFromMysql so live notes-store smoke-import still works. */
...
/** Called by live src/notes-store.js after schema setup. Must stay a named export. */
...
export async function getOverview() {
  const db = await loadDb();
  if (db?.query && typeof db.dbMode === "function" && db.dbMode() === "mysql") {
    const [cardRows] = await db.query(
    ...
    if (cardRows.length || eventRows.length) {
      return payload(
        cardRows.map((row) => ({
          key: row.card_key,
          label: row.label,
