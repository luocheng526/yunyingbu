/* xm-fast-shell 0.1.660-shell-pin — HTML pin must match so people.js cache drops */
(function () {
  const ASSET_VER = "0.1.660";
  ...
  const HAN_CHILDREN = [
    ...
    { href: "/han/paid-center", label: "付费中心" },
    { href: "/han/recharge-rules", label: "充值规则" },
  ...
  function siderHtml() {
    return (
      ...
      '<p class="xm-version">v0.4.24</p></nav>'
    );
  }
  ...
  function collectReloadUrls() {
    ...
    if (id) {
      ...
      add("/shared/modules/" + id + ".js?v=" + ASSET_VER);
    }
  ...
  function isPeopleRoute(href) {
    const key = normalize(href || current || location.pathname);
    return key === "/people" || key === "/people/";
  }

  function parkPeopleHosts() {
    const onPeople = isPeopleRoute();
    ...
    Array.prototype.forEach.call(pages, function (el) {
      ...
      const mine = pane && pane.getAttribute("data-xm-href") === "/people";
      if (onPeople && mine) {
  ...
  function injectModuleScript(id, href, resolve) {
    ...
    script.src = document.querySelector('script[src^="' + src + '"]') ? src + "?r=" + Date.now() : src + "?v=" + ASSET_VER;
  ...
  function watchNoticeMask() {
    if (document.documentElement.getAttribute("data-xm-notice-watch") === "1") {
      return;
    }
    document.documentElement.setAttribute("data-xm-notice-watch", "1");
    if (!document.body) {
      return;
    }
